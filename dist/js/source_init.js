...
$(document).ready(function () {
  ...
  $('#btn-agregarcanal').on('click', function (e) {
    ...
    if (nombre.length < 3) {
      ...
      db.collection("canales").where("nombre", "==", nombre).get().then((querySnapshot) => {
        if (querySnapshot.docs && querySnapshot.docs.length) {
          $("#btn-agregarcanal").attr("disabled", false);
          swal("Ya existe un Canal con el mismo nombre!", "Elegí otro.", "error");
        }
        else {
          db.collection("canales").
            ...
            limit(1).get().then((querySnapshot) => {
              ...
              db.collection('canales').doc().set({
                ...
                swal("Ocurrió un error al crear el Canal", error.toString(), "error");
                $("#btn-agregarcanal").attr("disabled", false);
                console.log(error);
              });
            });
...
function loadCanales(mostrarEliminados) {
  $("#container-main").html(`
              <div class="row">
                <div class="col">
...
function loadRadio() {
  db.collection("radios").doc("radio").get().then((doc) => {
    ...
    <div class='row'>
      <div class="form-group col-md-8">
        <label for="link-radio" class="col-form-label">Link del Streaming:</label>
    ...
    if (doc.exists && doc.data().stream) {
      ...
      $("#switch-canal").click(function (e) {
        toggleRadioActiva($(this).is(':checked') == true ? true : false, doc.id);
        return false;
      });
...
function guardarCambios() {
  ...
  if (!link.length) {
    ...
    setChanged(false);
    db.collection("radios").doc("radio").set({
      stream: link,
      activa: true
    }).then(() => {
